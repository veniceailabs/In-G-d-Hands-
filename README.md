...
`/team-queue` is an intentionally unlinked owner console for reviewing only the requests a person consented to share. A person must provide an email or phone number and expressly consent before a request is stored, so the team has a way to reply; that contact detail is not used to create a wellness profile. After submitting, they can withdraw that request from the same browser session using a random one-time token; only a one-way hash of the token reaches the database. The console is not public, does not persist a password in the browser, uses a secure HTTP-only 15-minute session, and supports `new`, `in progress`, and `closed` triage states. Before using it, set `TEAM_STAFF_CREDENTIALS` in Vercel for Production and Preview to a comma-separated list of individual `username:password` accounts (each password at least 12 characters, longer is better). Never use the Supabase service-role key as any of these passwords or place any of these values in frontend code.

Each staff member signs in with their own username, and every status change on a request records which username made it, so the console now has lightweight per-person accountability instead of one shared password. This is still not full staff identity or audit logging: usernames and passwords live in a Vercel environment variable rather than a managed identity provider, there is no self-serve password reset or role tiering, and there is no immutable audit trail of every read. Before giving more people access at scale, replace this with an approved staff-identity provider, role-based access, response-time ownership, and an incident process. The queue does not send notifications or imply that someone is monitoring it continuously.
